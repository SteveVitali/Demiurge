...
import lastmile.cli.Commands._
import lastmile.persistence.{Database, Migrator}
...
object CliApp {
  ...
  def run(args: Array[String]): Int = {
    CommandParsers.parse(args) match {
      ...
      case Right(ParseResult(global, cmd)) =>
        val dbDir = global.repo.resolve(".lastmile")
        Files.createDirectories(dbDir)
        val dbPath = dbDir.resolve("lastmile.db")
  ...
  private def printHelp(): Unit = {
    System.out.println(
      """lastmile — Last-Mile Web Development Automation Platform
        |
        |Usage: lastmile [global-flags] <command> [command-flags]
        |
        |Global Flags:
        ...
        |  init-manifest       Generate a lastmile.yaml manifest
        |
        |Run 'lastmile <command> --help' for command-specific help.
      """.stripMargin)
  }
}
