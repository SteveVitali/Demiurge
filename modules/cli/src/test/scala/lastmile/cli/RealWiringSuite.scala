...
import lastmile.orchestrator.RunTransitionManager
import lastmile.api.EventStream
import lastmile.requirements.RequirementsFile
import lastmile.selectors.SelectorsFile
import lastmile.cli.Commands.WorkerBrowserExecutor
...
class RealWiringSuite extends FunSuite {
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    tmpDir = Files.createTempDirectory("lastmile-wiring-test")
  ...
  test("RepoInspectorImpl inspects a real repo with package.json and lastmile.yaml") {
    ...
    Files.writeString(tmpDir.resolve("lastmile.yaml"),
      """version: 1
        |app:
        |  type: api
    ...
    assert(report.manifestsFound.exists(_.manifestType == "lastmile"), "Should find lastmile.yaml")
  ...
  // --- Test: EnvironmentPlannerImpl plans from lastmile.yaml manifest ---

  test("EnvironmentPlannerImpl produces real plan from lastmile.yaml") {
    Files.writeString(tmpDir.resolve("package.json"),
      """{"name":"test","scripts":{"start":"node server.js"},"dependencies":{"express":"^4.0.0"}}""")
    Files.writeString(tmpDir.resolve("lastmile.yaml"),
      """version: 1
    ...
    assert(plan.warnings.isEmpty || !plan.warnings.exists(_.contains("No valid lastmile.yaml")),
      "Should NOT warn about missing lastmile.yaml")
  }
  ...
  test("RunTransitionManager event listener receives state_transition events") {
    ...
    val ctx = lastmile.orchestrator.RunContext(
  ...
  test("ArtifactSinkImpl writes artifact files to disk") {
    ...
    val sink = new lastmile.artifact.ArtifactSinkImpl(artRoot)
  ...
  test("EvidenceCollectorImpl writes final report artifact") {
    ...
    java.nio.file.Files.createDirectories(artRoot)
    val sink = new lastmile.artifact.ArtifactSinkImpl(artRoot)
    val collector = new lastmile.artifact.EvidenceCollectorImpl(sink)
  ...
  test("WorkerBrowserExecutor handles missing worker gracefully") {
    ...
    val wpm = new lastmile.worker.WorkerProcessManager(fakeScript)
    ...
    val bv = lastmile.verification.BrowserFlowVerifier(
    ...
    assert(result.outcome.isInstanceOf[lastmile.verification.VerifierOutcome.Error],
