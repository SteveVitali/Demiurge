...
// Phase 8: Tests for fixture manifest parsing — exercises real demiurge.yaml files
class FixtureManifestSuite extends munit.FunSuite {
  ...
  test("simple-node-http demiurge.yaml parses correctly") {
    val manifest = fixturesRoot.resolve("simple-node-http/demiurge.yaml")
  ...
  test("compose-app demiurge.yaml parses correctly") {
    val manifest = fixturesRoot.resolve("compose-app/demiurge.yaml")
    if (Files.exists(manifest)) {
      ManifestParser.parseFile(manifest) match {
        case ManifestParser.ParseSuccess(m) =>
          assertEquals(m.version, 1)
