...
// Spec §5: Parse demiurge.yaml into DemiurgeManifest.
// Phase 3 only needs the fields required for runtime realization.
object ManifestParser {

  sealed trait ParseResult
  case class ParseSuccess(manifest: DemiurgeManifest) extends ParseResult
  ...
  private def parseRoot(root: Map[String, Any]): ParseResult = {
    ...
    if (errors.nonEmpty) ParseFailure(errors.toList)
      ...
      case Some(a) => ParseSuccess(DemiurgeManifest(version, a, services, fixtures, auth, verification, inference, policies, observability))
