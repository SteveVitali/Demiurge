...
class RuntimeSupervisorSuite extends FunSuite {
  ...
  test("writes PID files for script services") {
    withTempDir { root =>
      ...
      try {
        result match {
          case RuntimeSupervisor.BootSuccess(_) =>
            val pidFile = root.resolve(".demiurge").resolve("pids").resolve("pid-svc.pid")
  ...
  test("removes PID files on teardown") {
    withTempDir { root =>
      ...
      result match {
        case RuntimeSupervisor.BootSuccess(_) =>
          val pidFile = root.resolve(".demiurge").resolve("pids").resolve("cleanup-svc.pid")
        ...
        try {
          ...
          try {
            new ProcessBuilder("docker", "compose", "-f", root.resolve("compose.yaml").toString,
              "-p", s"demiurge-compose-", "down", "--remove-orphans")
