...
class EnvironmentPlannerSuite extends FunSuite {
  ...
  test("builds RuntimePlan from manifest") {
    withTempDir { root =>
      ...
      val manifestRef = ManifestRef("demiurge", "demiurge.yaml", parsedSuccessfully = true, parseErrors = Nil)
  ...
  test("plans from inspection when no manifest") {
    withTempDir { root =>
      ...
      assert(plan.warnings.exists(_.contains("No valid demiurge.yaml")))
      assertEquals(plan.services.size, 1)
