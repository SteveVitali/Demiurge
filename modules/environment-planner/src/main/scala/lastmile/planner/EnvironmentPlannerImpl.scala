...
// Produces a RuntimePlan from parsed lastmile.yaml + basic RepoInspectionReport.
...
object EnvironmentPlannerImpl extends EnvironmentPlanner {
  ...
  override def plan(runId: String, inspection: RepoInspectionReport, requirements: RequirementGraph): RuntimePlan = {
    // If a lastmile manifest was found and parsed successfully, use it
    val manifestRef = inspection.manifestsFound.find(m => m.manifestType == "lastmile" && m.parsedSuccessfully)

    // Try to load and parse manifest from the repo root
    val manifestOpt = manifestRef.flatMap { _ =>
      val manifestPath = inspection.repoRoot.resolve("lastmile.yaml")
  ...
  private def planFromManifest(runId: String, manifest: LastmileManifest, inspection: RepoInspectionReport): RuntimePlan = {
  ...
  private def planFromInspection(runId: String, inspection: RepoInspectionReport): RuntimePlan = {
    val warnings = scala.collection.mutable.ListBuffer[String]()
    warnings += "No valid lastmile.yaml found; planning from inspection hints only"
